...
namespace franka_example_controllers
{
  ...
  bool CartesianImpedanceExampleController::init(hardware_interface::RobotHW *robot_hw, // 硬件对象
                                                 ros::NodeHandle &node_handle           // 节点句柄
  )
  {
    std::vector<double> cartesian_stiffness_vector; // 笛卡尔刚度向量
    std::vector<double> cartesian_damping_vector;   // 笛卡尔阻尼向量

    // 订阅消息节点，获取机器人姿态，回调处理函数在文档最后面定义
    sub_equilibrium_pose_ = node_handle.subscribe(
        "equilibrium_pose", 20, &CartesianImpedanceExampleController::equilibriumPoseCallback, this,
        ros::TransportHints().reliable().tcpNoDelay());

    // 下面这一部分是在参数服务器(.yaml)中获取参数，如果获取失败，就会报错
    std::string arm_id; // 机械臂id
    ...
    // 设置回调函数，回调函数定义在文件末尾，参数更改是服务器被调用
                                                   ...
                                                   const ros::Duration & /*period*/)
  {
    ...
    Eigen::Map<Eigen::Matrix<double, 7, 1>> coriolis(coriolis_array.data()); // 克里奥利
    Eigen::Map<Eigen::Matrix<double, 6, 7>> jacobian(jacobian_array.data()); // jacobian矩阵
    Eigen::Map<Eigen::Matrix<double, 7, 1>> q(robot_state.q.data());         // 关节角度
    Eigen::Map<Eigen::Matrix<double, 7, 1>> dq(robot_state.dq.data());       // 关节角速度
    Eigen::Map<Eigen::Matrix<double, 7, 1>> tau_J_d(                         // NOLINT (readability-identifier-naming)
        robot_state.tau_J_d.data());
    Eigen::Affine3d transform(Eigen::Matrix4d::Map(robot_state.O_T_EE.data())); // 当前末端放射变换矩阵
    Eigen::Vector3d position(transform.translation());                          // 当前末端位置
    Eigen::Quaterniond orientation(transform.rotation());                       // 当前末端姿态
    ...
    // error的前三个元素是位置误差，后三个元素是姿态误差，所以一个从队首，一个从队尾
    // 对四元数的用法很奇怪
    ...
    // 求jacobian的伪逆,存放在jacobian_transpose_pinv中
    Eigen::MatrixXd jacobian_transpose_pinv;
    pseudoInverse(jacobian.transpose(), jacobian_transpose_pinv);

    // Cartesian PD control with damping ratio = 1
    // 任务空间 PD 控制 J^T(-K_{car} *error_{task} - D_{car} * J * dq)
    ...
    // Desired torque
    // 最终输出力矩就是两个力矩与克里奥利力的和
    tau_d << tau_task + tau_nullspace + coriolis;

    // Saturate torque rate to avoid discontinuities
    // 饱和扭矩率，以避免不连续，处理函数在下面
    ...
    // target by filtering
    // 动态更新参数 filter_params_ 是一个滤波器，用于平滑参数
    ...
    // 在这里整个过程中没有修改过 position_d_target_ 使其一直保持初始状态，可以通过修改这个任务目标实现不同的任务指令
  }
